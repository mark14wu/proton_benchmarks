#!/usr/bin/env python3
"""
Standalone training script for TinyLlama-1.1B-Chat model using Unsloth
This script demonstrates fine-tuning TinyLlama-1.1B-Chat with LoRA adapters
"""
...
def prepare_dataset(tokenizer, num_samples=1000):
    ...
    # 2) Set TinyLlama chat template (uses zephyr/chatml format)
    tokenizer = get_chat_template(tokenizer, chat_template="zephyr")
...
def initialize_model():
    """
    Initialize TinyLlama-1.1B-Chat model with LoRA adapters
    ...
    # TinyLlama is a 1.1B model, so we use smaller settings
...
def create_trainer(model, tokenizer, train_dataset):
    ...
    trainer = SFTTrainer(
        ...
        args=SFTConfig(
            ...
            # Other settings
            seed=3407,
            output_dir="./tinyllama_chat_outputs",
...
def train_with_profiling(trainer, profiling_mode="none", model_name="tinyllama_chat"):
    """
    ...
    if profiling_mode == "proton":
        ...
        session_id = proton.start(name=f"tinyllama_{model_name}", context="shadow")
        trainer.train()
        proton.finalize(session_id)
        print(f"Proton profile saved as tinyllama_{model_name}.hatchet")
        ...
        trace_file = f"tinyllama_trace_{model_name}.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Fine-tune TinyLlama-1.1B-Chat model using Unsloth and LoRA"
    )
    parser.add_argument(
        "--profiling",
        type=str,
    ...
    print("="*60)
    print("TinyLlama-1.1B-Chat Fine-tuning with Unsloth")
    print("="*60)
    print(f"Configuration:")
    print(f"  - Model: unsloth/tinyllama-chat")
    ...
    # model.save_pretrained("./tinyllama_chat_lora_adapters")
    # tokenizer.save_pretrained("./tinyllama_chat_lora_adapters")
    
    # Optional: Merge and save full model
    # print("Merging and saving full model...")
    # model.save_pretrained_merged("./tinyllama_chat_merged", tokenizer, save_method="merged_16bit")
