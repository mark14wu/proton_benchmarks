#!/usr/bin/env python3
"""
Standalone training script for SmolLM2.1-7B-Instruct model using Unsloth
This script demonstrates fine-tuning SmolLM2.1-7B-Instruct with LoRA adapters
"""
...
def prepare_dataset(tokenizer, num_samples=1000):
    ...
    # 2) Set SmolLM chat template (using chatml format)
...
def initialize_model():
    """
    Initialize SmolLM2.1-7B-Instruct model with LoRA adapters
    
    Returns:
        model: Model with LoRA adapters
        tokenizer: Tokenizer for the model
    """
    print("Initializing SmolLM2.1-7B-Instruct model...")
    ...
    # SmolLM2.1-7B is a 1.7B model, so we use moderate settings
...
def create_trainer(model, tokenizer, train_dataset):
    ...
    trainer = SFTTrainer(
        ...
        args=SFTConfig(
            ...
            # Other settings
            seed=3407,
            output_dir="./smollm2_1_7b_outputs",
    ...
    # Apply response-only training for SmolLM format
    trainer = train_on_responses_only(
        trainer,
        instruction_part="<|im_start|>",
...
def train_with_profiling(trainer, profiling_mode="none", model_name="smollm2_1_7b_instruct"):
    """
    ...
    if profiling_mode == "proton":
        ...
        session_id = proton.start(name=f"smollm2_1_7b_{model_name}", context="shadow")
        ...
        print(f"Proton profile saved as smollm2_1_7b_{model_name}.hatchet")
        ...
        trace_file = f"smollm2_1_7b_trace_{model_name}.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Fine-tune SmolLM2.1-7B-Instruct model using Unsloth and LoRA"
    )
    parser.add_argument(
        "--profiling",
        type=str,
    ...
    print("="*60)
    print("SmolLM2.1-7B-Instruct Fine-tuning with Unsloth")
    print("="*60)
    print(f"Configuration:")
    print(f"  - Model: unsloth/SmolLM2-1.7B-Instruct")
    ...
    # model.save_pretrained("./smollm2_1_7b_lora_adapters")
    # tokenizer.save_pretrained("./smollm2_1_7b_lora_adapters")
    
    # Optional: Merge and save full model
    # print("Merging and saving full model...")
    # model.save_pretrained_merged("./smollm2_1_7b_merged", tokenizer, save_method="merged_16bit")
