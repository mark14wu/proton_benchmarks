#!/usr/bin/env python3
"""
Standalone training script for Zephyr-7B-SFT model using Unsloth
This script demonstrates fine-tuning Zephyr-7B-SFT with LoRA adapters
"""
...
def initialize_model():
    ...
    """
    print("Initializing Zephyr-7B-SFT model...")
    ...
    # Zephyr-7B is based on Mistral-7B architecture
...
def train_with_profiling(trainer, profiling_mode="none", model_name="zephyr_sft"):
    """
    ...
    if profiling_mode == "proton":
        ...
        session_id = proton.start(name=f"zephyr_sft_{model_name}", context="shadow")
        trainer.train()
        proton.finalize(session_id)
        print(f"Proton profile saved as zephyr_sft_{model_name}.hatchet")
        ...
        trace_file = f"zephyr_sft_trace_{model_name}.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Fine-tune Zephyr-7B-SFT model using Unsloth and LoRA"
    )
    parser.add_argument(
        "--profiling",
        type=str,
    ...
    print("="*60)
    print("Zephyr-7B-SFT Fine-tuning with Unsloth")
    print("="*60)
    print(f"Configuration:")
    print(f"  - Model: unsloth/zephyr-sft")
    ...
    # model.save_pretrained("./zephyr_sft_lora_adapters")
    # tokenizer.save_pretrained("./zephyr_sft_lora_adapters")
    
    # Optional: Merge and save full model
    # print("Merging and saving full model...")
    # model.save_pretrained_merged("./zephyr_sft_merged", tokenizer, save_method="merged_16bit")
