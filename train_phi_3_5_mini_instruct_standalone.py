#!/usr/bin/env python3
"""
Standalone training script for Phi-3.5-Mini-Instruct model using Unsloth
This script demonstrates fine-tuning Phi-3.5-Mini-Instruct with LoRA adapters
"""
...
def initialize_model():
    """
    Initialize Phi-3.5-Mini-Instruct model with LoRA adapters
    
    Returns:
        model: Model with LoRA adapters
        tokenizer: Tokenizer for the model
    """
    print("Initializing Phi-3.5-Mini-Instruct model...")
    ...
    # Phi-3.5-mini is a 3.8B model, so we use moderate settings
    model = FastModel.get_peft_model(
        model,
        r=8,  # LoRA rank
        target_modules=["q_proj", "k_proj", "v_proj", "o_proj",
                       "gate_up_proj", "down_proj"],  # Phi-3 uses gate_up_proj
...
def create_trainer(model, tokenizer, train_dataset):
    ...
    trainer = SFTTrainer(
        ...
        args=SFTConfig(
            ...
            # Other settings
            seed=3407,
            output_dir="./phi_3_5_mini_outputs",
    ...
    # Apply response-only training for Phi-3 format
...
def train_with_profiling(trainer, profiling_mode="none", model_name="phi_3_5_mini_instruct"):
    """
    ...
    if profiling_mode == "proton":
        ...
        session_id = proton.start(name=f"phi_3_5_mini_{model_name}", context="shadow")
        trainer.train()
        proton.finalize(session_id)
        print(f"Proton profile saved as phi_3_5_mini_{model_name}.hatchet")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Fine-tune Phi-3.5-Mini-Instruct model using Unsloth and LoRA"
    )
    parser.add_argument(
        "--profiling",
        type=str,
    ...
    print("="*60)
    print("Phi-3.5-Mini-Instruct Fine-tuning with Unsloth")
    print("="*60)
    print(f"Configuration:")
    print(f"  - Model: unsloth/Phi-3.5-mini-instruct")
    ...
    # model.save_pretrained("./phi_3_5_mini_lora_adapters")
    # tokenizer.save_pretrained("./phi_3_5_mini_lora_adapters")
    
    # Optional: Merge and save full model
    # print("Merging and saving full model...")
    # model.save_pretrained_merged("./phi_3_5_mini_merged", tokenizer, save_method="merged_16bit")
