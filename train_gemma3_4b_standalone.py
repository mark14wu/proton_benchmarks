#!/usr/bin/env python3
"""
Standalone training script for Gemma3-4B model using Unsloth
This script demonstrates fine-tuning Gemma3-4B with LoRA adapters
"""
...
def initialize_model():
    """
    Initialize Gemma3-4B model with LoRA adapters
    
    Returns:
        model: Model with LoRA adapters
        tokenizer: Tokenizer for the model
    """
    print("Initializing Gemma3-4B model...")
    ...
    model = FastModel.get_peft_model(
        model,
        finetune_vision_layers=False,  # Gemma is text-only
...
def create_trainer(model, tokenizer, train_dataset):
    ...
    # Apply response-only training for Gemma-3 format
...
def train_with_profiling(trainer, profiling_mode="none", model_name="gemma3_4b"):
    """
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Fine-tune Gemma3-4B model using Unsloth and LoRA"
    )
    parser.add_argument(
        "--profiling",
        type=str,
    ...
    print("="*60)
    print("Gemma3-4B Fine-tuning with Unsloth")
    print("="*60)
    ...
    # print("Saving model...")
    # model.save_pretrained("./gemma3_4b_finetuned")
    # tokenizer.save_pretrained("./gemma3_4b_finetuned")
